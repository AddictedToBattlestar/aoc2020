...
export default class Day5BoardingPassParser extends Service.extend({
  ...
  public findSeatId(boardingPassSeatInformation: string): number {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      console.log(`-Day 5- Position ${i + 1}, value: ${boardingPassSeatInformation[i]}, lowerRowNumber: ${lowerRowNumber}, upperRowNumber: ${upperRowNumber}`);
    }
    ...
    console.log(`-Day 5- Boarding pass is for row number ${lowerRowNumber}`);
    ...
    for (let i = 7; i < 10; i++) {
      ...
      console.log(`-Day 5- Position ${i + 1}, value: ${boardingPassSeatInformation[i]}, leftMostSeatNumber: ${leftMostSeatNumber}, rightMostSeatNumber: ${rightMostSeatNumber}`);
    }
    ...
    console.log(`-Day 5- Boarding pass is for seat number ${leftMostSeatNumber}`);
    const seatId: number = lowerRowNumber * 8 + leftMostSeatNumber;
    console.log(`-Day 5- The seat ID for ${boardingPassSeatInformation} was found to be ${seatId}`);
    return seatId;
  }
  ...
  private findGapsInSeating(seatIds: Array<number>) {
    ...
    for (let i = 1; i < sortedSeatIds.length; i++) {
      if (sortedSeatIds[i] - sortedSeatIds[i - 1] !== 1) {
        console.log(`-Day 5- A seat gap was found for seat ID's ${sortedSeatIds[i - 1]} & ${sortedSeatIds[i]}`);
      }
    }
  }
}
