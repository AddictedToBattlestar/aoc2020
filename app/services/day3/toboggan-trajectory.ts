...
export default class Day3TobogganTrajectory extends Service.extend({
  ...
  public runWithSlope(mapData: Array<string>, horizontalSlope: number, verticalSlope: number): number {
    ...
    console.log(`-Day 3- outOfBounds: ${outOfBounds.x},${outOfBounds.y}`);
    ...
    while (currentPosition.y < outOfBounds.y) {
      if (mapData[currentPosition.y][currentPosition.x] === '#') {
        ...
        console.log(`-Day 3- current position = ${mapData[currentPosition.y][currentPosition.x]} (${currentPosition.x + 1},${currentPosition.y + 1}), tree hit (${currentCountOfTreesHit})`);
      } else {
        if(mapData[currentPosition.y][currentPosition.x] !== '.'){
          console.log("-Day 3- ERROR, invalid value encountered");
        }
        console.log(`-Day 3- current position = ${mapData[currentPosition.y][currentPosition.x]} (${currentPosition.x + 1},${currentPosition.y + 1})`);
      }
