...
module('Unit | Service | day11/seating-system', function(hooks) {
  setupTest(hooks);
  let service: any;

  hooks.beforeEach(function () {
    service = this.owner.lookup('service:day11/seating-system');
  });

  test('it solves the example provided from aoc for part 1', function (assert) {
    const result = service.calculateSeatCount(exampleData, 4);
    assert.equal(result, 37);
  });

  test('it solves for part 1 for day 11', function (assert) {
    const result = service.runUntilNoChangeDetected(sourceData);
    assert.ok(result);
    if (result != 0) {
      console.info(`-Day 11- The answer to part 1 is: ${result}\n`);
    }
  });
