...
module('Unit | Service | day11/seating-system-v2', function(hooks) {
  setupTest(hooks);
  let service: any;

  hooks.beforeEach(function () {
    service = this.owner.lookup('service:day11/seating-system-v2');
  });

  test('it solves the example provided from aoc for part 2 for day 11', function (assert) {
    const result = service.runUntilNoChangeDetected(exampleData);
    assert.equal(result, 26);
  });

  test('it solves for part 2 for day 11', function (assert) {
    const result = service.runUntilNoChangeDetected(sourceData);
    assert.ok(result);
    if (result != 0) {
      console.info(`-Day 11- The answer to part 2 is: ${result}\n`);
    }
  });
