...
module('Unit | Service | day6/customs-declaration-processing', function(hooks) {
  setupTest(hooks);
  let service: any;

  hooks.beforeEach(function () {
    service = this.owner.lookup('service:day6/customs-declaration-processing');
  });

  test('it solves the example provided from aoc for part 1', function (assert) {
    const result = service.countValidCustomDeclarationsByGroup(exampleData);
  ...
  test('it solves the example provided from aoc for part 2', function (assert) {
    const result = service.countValidCustomDeclarationsByGroup(exampleData, true);
